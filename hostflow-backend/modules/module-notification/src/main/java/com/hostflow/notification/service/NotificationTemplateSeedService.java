...
public class NotificationTemplateSeedService {
    ...
    public void seedDefaults() {
        ...
        seedIfMissing("rental_reservation_owner", NotificationChannel.EMAIL, "New reservation request on {{property_name}}",
                "A tenant requested to reserve {{property_name}} starting {{move_in_date}} for {{months}} month(s) "
                        + "at {{monthly_rent}}/month. Approve or decline it from your Rental tab.");
        seedIfMissing("maintenance_request_owner", NotificationChannel.EMAIL, "New maintenance issue on {{property_name}}",
